              ...
              <div class="space-y-6 md:space-x-4 md:space-y-0 form">
                ...
                <div class="flex-col w-full space-y-7 half-on-md">
                  ...
                  <div class="flex space-x-4">
                    ...
                    >
                      <span
                        v-if="!newCategoryField"
                        class="pb-1 mx-auto text-2xl"
                        >New</span
                      >
                      <svg
                        v-else
...
export default defineComponent({
  ...
  methods: {
    submitNewCategory(newCategory: string) {
      this.$axios
        .post("/new-category", { category: newCategory })
        .then((data) => {
          console.log(data);
          this.getCategories();
        })
    ...
    getCategories() {
      this.$axios
        ...
        .then((res) => {
          this.categories = res.data.categories.map(
          ...
          switch (this.optionType) {
            case "daily":
              (this as any).$store.dispatch(
                "plans/currentPlans/fetchTodaysPlans"
              );
              (this as any).$store.dispatch("plans/dailyPlans/fetchPlans");
              break;
            case "monthly":
              (this as any).$store.dispatch(
                "plans/currentPlans/fetchCurrentMonthPlans"
              );
              (this as any).$store.dispatch("plans/monthlyPlans/fetchPlans");
              break;
            case "yearly":
              (this as any).$store.dispatch(
                "plans/currentPlans/fetchCurrentYearPlans"
              );
